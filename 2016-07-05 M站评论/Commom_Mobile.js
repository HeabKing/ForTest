...
//var http_url = "http://api.comment.zxxk.com/"; 引用页赋值
...
function commentlistonload() {
    ...
    comment.getcommentlist(comment_infoid, 15, 1, '"[IsHot] desc,[PraiseCount] desc,[CommentTime]"', function (j) {
        ...
        if (window.location.host !== "m.zxxk.com") {
            while (htm.match("http://m.zxxk.com/comment/commentinfo")) {
                htm = htm.replace("http://m.zxxk.com/comment/commentinfo", "http://" + window.location.host + "/comment/commentinfo");
            }
        }
        commentroot().append(htm);
    });
    //去掉maxlength onchange onkeydown onkeyup
    $('.comment-int-area textarea').removeAttr('maxlength').removeAttr('onchange').removeAttr('onkeydown').removeAttr('onkeyup');
    //增加评论字数限制框
    $('.comment-cz-brns').prepend('<div class="num" id="comments-num">180</div>');
    //评论框字数限制
    xkw.system.limitTextarea();
    $('.comment-int-area textarea').limitTextarea({
...
function addcomment() {
    ...
    var remain = Number($('#comments-num').html());
    if (!(remain >= 0)) {//评论字数过长时；
        alert("超过180个字了");
        return;
    }
...
function comment_login() {
    if (typeof (loginpath) != "undefined") {
        window.location.href = loginpath + window.location.href;
    } else {
        alert("请先登录!");
    }
}